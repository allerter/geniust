...
from constants import DEVELOPERS, BOT_TOKEN, GENIUS_TOKEN, DATABASE_URL, SERVER_PORT, SERVER_ADDRESS
import traceback
import sys
import api
import re
...
(SELECT_ACTION, GENIUS, TELEGRAPH, CHECK_GENIUS_ALBUM, CUSTOMIZE_GENIUS, TYPING,
 OPTION1, OPTION2, OPTION3, OPTION4, IDENTIFIER, AUTHOR_NAME, AUTHOR_URL, ANNOTATIONS, LYRICS_LANG, START_OVER,
 INCLUDE, TYPING_ALBUM, STATISTICS, CURRENT_LEVEL, CREATE_ACCOUNT, CREATE_PAGES, CUSTOMIZE_TELEGRAPH,
 LOGIN, DESCRIPTION, SELF, CHECK_GENIUS_SONG, TYPING_SONG, TYPING_ACCOUNT, DOWNLOAD_ALBUM) = range(30)
...
class WebhookThread(threading.Thread):
    """start a web hook server to respond to cron jobs that keep the bot from going to sleep in Heroku free plan"""
...
def genius(update, context):
    ...
    buttons = [[
        InlineKeyboardButton(text='Song Lyrics', callback_data=str(CHECK_GENIUS_SONG)),
        InlineKeyboardButton(text='Album Lyrics', callback_data=str(CHECK_GENIUS_ALBUM))
    ],
        [
        InlineKeyboardButton(text='Customize Lyrics', callback_data=str(CUSTOMIZE_GENIUS))
    ],
        [
...
def inline_query(update, context):
    """Handle inline queries and /start commands that contain arguments (IDs for fetching songs and albums)"""
    ...
    if update.inline_query:
        ...
        if text[0] == '':
            ...
            answer_text = ('Normally inline mode will look for songs. But if you want to search album lyrics,'
                           'just add a "@" at the end of your search. For example:\n'
                           'Searching songs:\n<code>@genius_the_bot some title</code>\n'
                           'Searching albums:\n<code>@genius_the_bot some title @</code>')
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(text='Search...', switch_inline_query_current_chat='')]])
            answer = InlineQueryResultArticle(id=uuid4(), title=text, description=description,
                                              input_message_content=InputTextMessageContent(answer_text, parse_mode='HTML'),
        ...
        if len(text) == 1:
            ...
            for i in range(n_hits):
                ...
                button_url = create_deep_linked_url(context.bot.get_me().username, str(f'{song_id}'))
                keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(text='Get lyrics', url=button_url)],
                                                 [InlineKeyboardButton(text='Search...', switch_inline_query_current_chat='')]])
                answer = InlineQueryResultArticle(id=uuid4(), title=text, thumb_url=search_hit['song_art_image_thumbnail_url'],
                                                  input_message_content=InputTextMessageContent(answer_text, parse_mode='HTML'),
                                                  reply_markup=keyboard, description=description)
                # It's possible to provide results that are captioned photos of the song cover art, but that requires using InlineQueryResultPhoto
                # and user might not be able to choose the right song this way, since all they get is only the cover arts of the hits.
                # answer = InlineQueryResultPhoto(id=uuid4(), photo_url=search_hit['song_art_image_url'],
            ...
            for i, hit in enumerate(json_search['sections'][4]['hits']):
                ...
                # can't use string_funcs.format_title here because of the album name in response
                ...
                button_url_1 = create_deep_linked_url(context.bot.get_me().username, str(f'{artist_id}_{album_id}_{OPTION1}'))
                button_url_2 = create_deep_linked_url(context.bot.get_me().username, str(f'{artist_id}_{album_id}_{OPTION2}'))
                button_url_3 = create_deep_linked_url(context.bot.get_me().username, str(f'{artist_id}_{album_id}_{OPTION3}'))
                keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(text='PDF', url=button_url_1)],
                                                 [InlineKeyboardButton(text='ZIP', url=button_url_2)],
                                                 [InlineKeyboardButton(text='TELEGRA.PH', url=button_url_3)],
                                                 [InlineKeyboardButton(text='Search...', switch_inline_query_current_chat='')]])
                answer = InlineQueryResultArticle(id=uuid4(), title=text, thumb_url=album['cover_art_thumbnail_url'],
                                                  input_message_content=InputTextMessageContent(answer_text, parse_mode='HTML'),
                                                  reply_markup=keyboard, description=description)
        ...
        if len(hits) == 0:
            ...
            keyboard = keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(text='Start a chat', url=button_url)]])
            answer_text = f'Search in a chat with the bot:'
            hits = [InlineQueryResultArticle(id=uuid4(), title='Nothing Found...',
                                             input_message_content=InputTextMessageContent(answer_text),
                                             reply_markup=keyboard)]
        update.inline_query.answer(hits)
    # this part is for when users who have used inline queries, click /start to get song or album lyrics
    elif context.args:
        ...
        if len(data) == 1:
            p = threading.Thread(target=print_lyrics,
                                 args=(context.bot, context.user_data, int(data[0]), genius_api,))
            # p.daemon = True
            p.start()
            p.join()
        else:
            ...
            page = 1
            while not found:
                res = genius_api._make_public_request(f'artists/{artist_id}/albums', page)
                for album in res['albums']:
                    ...
                    page += 1
            p = threading.Thread(target=get_album, args=(GENIUS_TOKEN, album_url,
...
def check_album_link(update, context):
        ...
        # the message is a name (not a valid link)
            ...
            for i, hit in enumerate(json_search['sections'][4]['hits']):
                ...
                if 'Genius' in album['name_with_artist']:
                    ...
                    text = album['full_title'].split(' by ')
                    text = f'{text[1]} - {text[0]}'
                hits.append([InlineKeyboardButton(text=text, callback_data=str(f'{artist_id}_{album_id}'))])
    ...
    # user has chosen one of the hits or selected Album Lyrics from the main menu
        ...
        if len(data) == 1:
            update.callback_query.answer()
            update.callback_query.edit_message_text(text='Send me an album link or album name to search.')
            return TYPING_ALBUM
    ...
    keyboard = InlineKeyboardMarkup(buttons)
    context.bot.send_message(text='Choose a format', chat_id=chat_id, reply_markup=keyboard)
...
def get_album_format(update, context):
    ...
    p = threading.Thread(target=get_album, args=(GENIUS_TOKEN, context.user_data['album_url'],
...
def get_album(GENIUS_TOKEN, link, message, user_data, album_format, bot):
    ...
    # try clause for the callback query users and the except clause for inline query users
    try:
...
def print_lyrics(bot, user_data, song_id, genius_api, message_id=None):
    ...
    if message_id:
        ...
        message_id = bot.send_message(chat_id=user_data['chat_id'], text='getting lyrics...')['message_id']
    json_song = api._API._make_api_request(genius_api, (song_id, 'song'))
    lyrics = api._API._scrape_song_lyrics_from_url(genius_api, json_song['song']['url'], song_id,
                                                   include_annotations=include_annotations,
                                                   telegram_song=True, lyrics_language=lyrics_language)[0]
    ...
    lyrics = re.sub(r'<(?:\/(?!(?:a|b|br|strong|em|i)>)[^>]*|(?!\/)(?!(?:a\s+[^\s>]+|b|br|strong|em|i)>)[^>]*)>', '', lyrics)
    # edit the message for callback query users
    if message_id:
        bot.edit_message_text(chat_id=user_data['chat_id'],
                              message_id=message_id, text="Here's the lyrics:")
    ...
    while sent < len_lyrics:
        text = lyrics[i * MAX_MESSAGE_LENGTH: (i * MAX_MESSAGE_LENGTH) + MAX_MESSAGE_LENGTH]
...
def check_song(update, context):
    ...
    if update.callback_query:
        if int(update.callback_query.data) == CHECK_GENIUS_SONG:
            update.callback_query.answer()
            update.callback_query.edit_message_text('Send me either a Genius song link, or a title to search')
            return TYPING_SONG
        else:
    ...
    # user has sent either a song link, a title to search or /song_lyrics
        ...
        if text == '/song_lyrics':
            if not context.user_data.get('lyrics_lang'):
                res = database(chat_id=chat_id, action='select')
                context.user_data.update(res)
            update.message.reply_text('Send me either a Genius song link, or a title to search')
            return TYPING_SONG
        parsed_url = urlparse(text)
        ...
        # user has sent a title (which is an invalid link)
            ...
            for i in range(n_hits):
                ...
                found_artist = search_hit['primary_artist']['name']
                text = string_funcs.format_title(found_artist, found_song)
                buttons.append([InlineKeyboardButton(text=text, callback_data=str(search_hit['id']))])
...
def customize_genius(update, context):
    ...
    try:
        update.callback_query.answer()
        update.callback_query.edit_message_text(text=text, reply_markup=keyboard, parse_mode='HTML')
...
def set_lyrics_language(update, context):
    ...
    buttons = [
        [InlineKeyboardButton(text='Only English (ASCII)', callback_data=str(OPTION1))],
        [InlineKeyboardButton(text='Only non-English (non-ASCII)', callback_data=str(OPTION2))],
        [InlineKeyboardButton(text='English + non-English', callback_data=str(OPTION3))],
        [InlineKeyboardButton(text='Back', callback_data=str(END))]
    ]
    keyboard = InlineKeyboardMarkup(buttons)
    ...
    if data >= OPTION1 and data <= OPTION3:
        ...
        database(chat_id=chat_id, action='update', data=ud['lyrics_lang'], update='lyrics_lang')
...
def choose_include_annotations(update, context):
    ...
    if data == OPTION1 or data == OPTION2:
        ...
        database(chat_id=chat_id, action='update', data=ud['include_annotations'], update='include_annotations')
        text = f'Updated your preferences.\n{text}'
        if ud.get('command'):
            if ud['command']:
...
def save_input(update, context):
    ...
    if update.effective_chat.username:
        ...
        text = f'User: <a href={update.effective_user.id}>{update.effective_user.first_name}</a>\n'
    text += update.message.text
    for developer in DEVELOPERS:
        context.bot.send_message(chat_id=developer, text=text, parse_mode='HTML')
    context.bot.send_message(chat_id=update.message.chat.id, text='Thanks for the feedback!')
    return END
...
def end_describing(update, context):
    ...
    if level == SELF:
        ...
        if keyboards[-1] == 'Done':
            context.bot.delete_message(chat_id=qd.message.chat.id, message_id=qd.message.message_id)
            return END
...
def main():
    ...
    my_states = [CallbackQueryHandler(genius, pattern='^' + str(GENIUS) + '$'),
                 CallbackQueryHandler(check_album_link, pattern='^' + str(CHECK_GENIUS_ALBUM) + '$'),
                 CallbackQueryHandler(check_song, pattern='^' + str(CHECK_GENIUS_SONG) + '$'),
                 CallbackQueryHandler(customize_genius, pattern='^' + str(CUSTOMIZE_GENIUS) + '$'),
                 CallbackQueryHandler(set_lyrics_language, pattern='^' + str(LYRICS_LANG) + '$'),
                 CallbackQueryHandler(choose_include_annotations, pattern='^' + str(INCLUDE) + '$'),
                 ]
    user_input = {
        TYPING_ALBUM: [MessageHandler(Filters.text & (~Filters.command), check_album_link)],
        TYPING_SONG: [MessageHandler(Filters.text & (~Filters.command), check_song)],
        TYPING: [MessageHandler(Filters.text & (~Filters.command), save_input)],
        ANNOTATIONS: [CallbackQueryHandler(choose_include_annotations, pattern='^(?!' + str(END) + ').*$')],
        LYRICS_LANG: [CallbackQueryHandler(set_lyrics_language, pattern='^(?!' + str(END) + ').*$')],
        CHECK_GENIUS_SONG: [CallbackQueryHandler(check_song, pattern='^(?!' + str(END) + ').*$')],
        CHECK_GENIUS_ALBUM: [CallbackQueryHandler(check_album_link, pattern='^(?!' + str(END) + ').*$')],
        DOWNLOAD_ALBUM: [CallbackQueryHandler(get_album_format, pattern='^(?!' + str(END) + ').*$')]
    }
    # main conversation handler
    main_menu_conv_handler = ConversationHandler(
        entry_points=[CommandHandler('start', genius, Filters.regex(r'^\D*$')),
                      CommandHandler("start", inline_query, Filters.regex(r'[\d]+'), pass_args=True)],
        ...
        fallbacks=[CallbackQueryHandler(end_describing, pattern='^' + str(END) + '$'),
    ...
    commands_conv_handler = ConversationHandler(
        ...
        states={
            SELECT_ACTION: [CallbackQueryHandler(check_album_link, pattern='^' + str(CHECK_GENIUS_ALBUM) + '$'),
                            CallbackQueryHandler(check_song, pattern='^' + str(CHECK_GENIUS_SONG) + '$')
                            ],
            **user_input
        },
    ...
    dp.add_handler(CommandHandler("start", inline_query, Filters.regex(r'[\d]+'), pass_args=True))
