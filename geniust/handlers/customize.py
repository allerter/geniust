...
from telegram import (
    InlineKeyboardMarkup as IBKeyboard,
    InlineKeyboardButton as IButton,
)

from constants import (
    INCLUDE, CUSTOMIZE_MENU,
    END, LYRICS_LANG, SELECT_ACTION,
...
def customize_menu(update, context):
    ...
    buttons = [
        [IButton('Lyrics Language', str(LYRICS_LANG))],
        [IButton('Annotations', str(INCLUDE))],
...
def lyrics_language(update, context):
    ...
    if update.message or int(update.callback_query.data) == LYRICS_LANG:
        ...
        buttons = [
            [IButton('Only English (ASCII)', str(OPTION1))],
            [IButton('Only non-English (non-ASCII)', str(OPTION2))],
            [IButton('English + non-English', str(OPTION3))],
            [IButton('Back', str(END))]
        ]
        keyboard = IBKeyboard(buttons)
